...
var $e = (s, e, t) => e in s ? Ie(s, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : s[e] = t;
...
const xe = 6048e5, Oe = 864e5, de = Symbol.for("constructDateFrom");
function $(s, e) {
...
function Ne(s, e, t) {
  const [a, r] = ke(
    ...
    s,
    e
  ), i = X(a), n = X(r), o = +i - he(i), c = +n - he(n);
  return Math.round((o - c) / Oe);
}
function Re(s, e) {
  const t = we(s, e), a = $(s, 0);
...
function Ue(s) {
  return !(!Fe(s) && typeof s != "number" || isNaN(+M(s)));
}
function qe(s, e) {
  const t = M(s, e == null ? void 0 : e.in);
...
function q(s) {
  return (e, t = {}) => {
    ...
    if (!i)
      return null;
    const n = i[0], o = a && s.parsePatterns[a] || s.parsePatterns[s.defaultParseWidth], c = Array.isArray(o) ? st(o, (h) => h.test(n)) : (
...
const ct = /^(\d+)(th|st|nd|rd)?/i, lt = /\d+/i, dt = {
  narrow: /^(b|a)/i,
...
function kt(s, e) {
  const t = M(s, e == null ? void 0 : e.in), a = +Q(t) - +Re(t);
...
function Ct(s, e) {
  var o, c, l, d;
  const t = Z(), a = (e == null ? void 0 : e.firstWeekContainsDate) ?? ((c = (o = e == null ? void 0 : e.locale) == null ? void 0 : o.options) == null ? void 0 : c.firstWeekContainsDate) ?? t.firstWeekContainsDate ?? ((d = (l = t.locale) == null ? void 0 : l.options) == null ? void 0 : d.firstWeekContainsDate) ?? 1, r = Ce(s, e), i = $((e == null ? void 0 : e.in) || s, 0);
  return i.setFullYear(r, 0, a), i.setHours(0, 0, 0, 0), G(i, e);
}
function Et(s, e) {
  const t = M(s, e == null ? void 0 : e.in), a = +G(t, e) - +Ct(t, e);
  return Math.round(a / xe) + 1;
}
function C(s, e) {
...
function pe(s, e) {
  return s % 60 === 0 ? (s > 0 ? "-" : "+") + C(Math.abs(s) / 60, 2) : z(s, e);
}
function z(s, e = "") {
  const t = s > 0 ? "-" : "+", a = Math.abs(s), r = C(Math.trunc(a / 60), 2), i = C(a % 60, 2);
  return t + r + e + i;
}
const me = (s, e) => {
  switch (s) {
  ...
  return i.replace("{{date}}", me(a, e)).replace("{{time}}", Ee(r, e));
}, St = {
  p: Ee,
  P: Pt
}, It = /^D+$/, $t = /^Y+$/, Mt = ["D", "DD", "YY", "YYYY"];
function Lt(s) {
  return It.test(s);
}
function Dt(s) {
...
function At(s, e, t) {
  const a = Bt(s, e, t);
  if (console.warn(a), Mt.includes(s)) throw new RangeError(a);
}
function Bt(s, e, t) {
...
const zt = /[yYQqMLwIdDecihHKkms]o|(\w)\1*|''|'(''|[^'])+('|$)|./g, Ot = /P+p+|P+|p+|''|'(''|[^'])+('|$)|./g, Wt = /^'([^]*?)'?$/, Nt = /''/g, Rt = /[a-zA-Z]/;
function B(s, e, t) {
  var d, h, u, g;
  const a = Z(), r = a.locale ?? Tt, i = a.firstWeekContainsDate ?? ((h = (d = a.locale) == null ? void 0 : d.options) == null ? void 0 : h.firstWeekContainsDate) ?? 1, n = a.weekStartsOn ?? ((g = (u = a.locale) == null ? void 0 : u.options) == null ? void 0 : g.weekStartsOn) ?? 0, o = M(s, t == null ? void 0 : t.in);
  if (!Ue(o))
    throw new RangeError("Invalid time value");
  let c = e.match(Ot).map((p) => {
    const b = p[0];
    if (b === "p" || b === "P") {
      const m = St[b];
      return m(p, r.formatLong);
    }
    return p;
  }).join("").match(zt).map((p) => {
    if (p === "''")
  ...
  return c.map((p) => {
    ...
    const b = p.value;
    (Dt(b) || Lt(b)) && At(b, e, String(s));
    const m = ue[b[0]];
    return m(o, b, r.localize, l);
  }).join("");
}
function Ft(s) {
  const e = s.match(Wt);
  return e ? e[1].replace(Nt, "'") : s;
}
function Ut(s, e) {
  return le(
    $(s, s),
...
function Vt(s) {
  try {
    ...
    if ((!e || typeof e != "string") && (e = (orca.state.locale || "zh-CN").startsWith("zh") ? "yyyy年MM月dd日" : "yyyy-MM-dd"), Ut(s))
      return "今天";
    if (Ht(s))
      return "昨天";
    if (qt(s))
...
function ka(s, e, t, a) {
  ...
  p.className = "dialog-width-display", p.style.cssText = `
    ...
    font-size: 14px;
    color: #666;
  `, p.textContent = `最小宽度: ${e}px`, l.oninput = () => {
    const v = parseInt(l.value), T = parseInt(g.value);
    v < T && (g.value = v.toString(), p.textContent = `最小宽度: ${v}px`), d.textContent = `最大宽度: ${v}px`;
    const x = parseInt(l.value), k = parseInt(g.value);
    t(x, k);
  }, g.oninput = () => {
    const v = parseInt(l.value), T = parseInt(g.value);
    T > v && (l.value = T.toString(), d.textContent = `最大宽度: ${T}px`), p.textContent = `最小宽度: ${T}px`;
    const x = parseInt(l.value), k = parseInt(g.value);
    t(x, k);
  ...
  b.className = "dialog-buttons", b.style.cssText = `
    display: flex;
  ...
  m.className = "btn btn-primary", m.textContent = "确定", m.style.cssText = K(), m.onclick = () => {
    const v = parseInt(l.value), T = parseInt(g.value);
    t(v, T), J(r);
  };
  const y = document.createElement("button");
  return y.className = "btn btn-secondary", y.textContent = "取消", y.style.cssText = K(), y.onclick = () => {
    a && a(), J(r);
  }, b.appendChild(m), b.appendChild(y), r.appendChild(b), r;
}
function Ca(s, e, t) {
  ...
  d.className = "btn btn-primary", d.textContent = "确定", d.style.cssText = K(), d.onclick = () => J(a);
  const h = document.createElement("button");
  return h.className = "btn btn-secondary", h.textContent = "取消", h.style.cssText = K(), h.onclick = () => {
    t(), J(a);
...
class Sa {
  ...
  constructor(e) {
    ...
    // 防抖函数实例（仅用于拖拽等非关键场景）
    f(this, "draggingDebounce", ga(async () => {
  ...
  /** 简单的日志方法 */
  log(e, ...t) {
    this.currentLogLevel >= S.INFO && re(e, ...t);
  }
  logError(e, ...t) {
    this.currentLogLevel >= S.ERROR && xa(e, ...t);
  }
  logWarn(e, ...t) {
    this.currentLogLevel >= S.WARN && Ta(e, ...t);
  }
  /**
   * 初始化性能优化器
   */
...
`);
  ...
  /** 调试日志 - 用于开发调试，记录一般信息 */
  debugLog(...e) {
    this.currentLogLevel >= S.DEBUG && re(e.join(" "), ...e);
  }
  /** 详细日志 - 仅在详细模式下启用，记录详细的调试信息 */
  verboseLog(...e) {
    this.currentLogLevel >= S.VERBOSE && re(e.join(" "), ...e);
  }
  /** 警告日志 - 记录警告信息，提醒潜在问题 */
  warn(...e) {
    this.logWarn(e.join(" "));
  ...
  async restoreDebugMode() {
    try {
      await this.storageService.getConfig(w.DEBUG_MODE, this.pluginName) && this.setLogLevel(S.VERBOSE);
    } catch {
    }
  }
  /**
...
`)[0].trim();
      ...
      try {
        ...
        d && d.type === 1 && (n = d.value), h && h.type === 1 && h.value && h.value.trim() ? (o = h.value, this.log(`🎨 使用用户自定义图标: ${o} (块ID: ${e})`)) : (this.showBlockTypeIcons || l === "journal") && (o = _(l), this.log(`🎨 使用块类型图标: ${o} (块类型: ${l}, 块ID: ${e})`));
      } catch (d) {
    ...
    try {
      ...
      if (this.addNewTabButton(), this.enableWorkspaces && this.addWorkspaceButton(), this.isFixedToTop) {
        ...
        g.forEach((b) => {
          ...
          if (!m) return;
          const v = this.getCurrentPanelTabs().find((T) => T.blockId === m);
          if (v) {
            let T, x, k = "normal";
            if (T = "var(--orca-tab-bg)", x = "var(--orca-color-text-1)", v.color)
              try {
                b.style.setProperty("--tab-color", v.color), (document.documentElement.hasAttribute("data-theme") ? document.documentElement.getAttribute("data-theme") === "dark" : document.documentElement.classList.contains("dark")) && b.style.setProperty(
            ...
            app-region: no-drag;
            pointer-events: auto;
          `, v.color && b.style.setProperty("--tab-color", v.color);
          }
        });
  ...
  async showWidthAdjustmentDialog() {
    try {
      if (this.isVerticalMode) {
        ...
        const e = this.horizontalTabMaxWidth, t = this.horizontalTabMinWidth, a = be(
          this.horizontalTabMaxWidth,
          this.horizontalTabMinWidth,
          async (r, i) => {
            this.horizontalTabMaxWidth = r, this.horizontalTabMinWidth = i, await this.createTabsUI();
            try {
              await this.saveLayoutMode();
            } catch (n) {
              this.error("保存宽度设置失败:", n);
            }
            this.log(`📏 标签宽度已实时调整: 最大${r}px, 最小${i}px`);
          },
          async () => {
    ...
    for (let a = 0; a < e.length; a++) {
      ...
      try {
        ...
        if (i) {
          ...
          o && o.type === 1 && (l = o.value), c && c.type === 1 && c.value && c.value.trim() ? d = c.value : d || (d = _(n)), r.blockType !== n || r.icon !== d || r.color !== l ? (e[a] = {
            ...r,
            blockType: n,
            icon: d,
            color: l
  ...
  createTabElement(e) {
    ...
    const r = this.isVerticalMode && !this.isFixedToTop, i = Kt(e, r, () => "", this.horizontalTabMaxWidth, this.horizontalTabMinWidth);
    t.style.cssText = i;
    const n = Jt();
   ...
   */
  rgbToOklch(e, t, a) {
    const r = e / 255, i = t / 255, n = a / 255, o = (ee) => ee <= 0.04045 ? ee / 12.92 : Math.pow((ee + 0.055) / 1.055, 2.4), c = o(r), l = o(i), d = o(n), h = c * 0.4124564 + l * 0.3575761 + d * 0.1804375, u = c * 0.2126729 + l * 0.7151522 + d * 0.072175, g = c * 0.0193339 + l * 0.119192 + d * 0.9503041, p = 0.2104542553 * h + 0.793617785 * u - 0.0040720468 * g, b = 1.9779984951 * h - 2.428592205 * u + 0.4505937099 * g, m = 0.0259040371 * h + 0.7827717662 * u - 0.808675766 * g, y = Math.cbrt(p), v = Math.cbrt(b), T = Math.cbrt(m), x = 0.2104542553 * y + 0.793617785 * v + 0.0040720468 * T, k = 1.9779984951 * y - 2.428592205 * v + 0.4505937099 * T, I = 0.0259040371 * y + 0.7827717662 * v - 0.808675766 * T, E = Math.sqrt(k * k + I * I), R = Math.atan2(I, k) * 180 / Math.PI, F = R < 0 ? R + 360 : R;
    return { l: x, c: E, h: F };
  }
  /**
   * OKLCH转RGB颜色空间
   */
  oklchToRgb(e, t, a) {
    const r = a * Math.PI / 180, i = t * Math.cos(r), n = t * Math.sin(r), o = e, c = i, l = n, d = o * o * o, h = c * c * c, u = l * l * l, g = 1.0478112 * d + 0.0228866 * h - 0.050217 * u, p = 0.0295424 * d + 0.9904844 * h + 0.0170491 * u, b = -92345e-7 * d + 0.0150436 * h + 0.7521316 * u, m = 3.2404542 * g - 1.5371385 * p - 0.4985314 * b, y = -0.969266 * g + 1.8760108 * p + 0.041556 * b, v = 0.0556434 * g - 0.2040259 * p + 1.0572252 * b, T = (E) => E <= 31308e-7 ? 12.92 * E : 1.055 * Math.pow(E, 1 / 2.4) - 0.055, x = Math.max(0, Math.min(255, Math.round(T(m) * 255))), k = Math.max(0, Math.min(255, Math.round(T(y) * 255))), I = Math.max(0, Math.min(255, Math.round(T(v) * 255)));
    return { r: x, g: k, b: I };
  }
  /**
   * 应用颜色调整（支持亮色和暗色模式）
  ...
  checkSettingsChange() {
    ...
    try {
      ...
      if (t.debugMode !== this.lastSettings.debugMode && (t.debugMode ? this.setLogLevel(S.VERBOSE) : this.setLogLevel(S.INFO), this.storageService.saveConfig(w.DEBUG_MODE, t.debugMode, this.pluginName).catch((a) => {
  ...
  showOrcaRenameInput(e) {
    ...
    let n = { x: "50%", y: "50%" };
    if (i) {
      const h = i.getBoundingClientRect(), u = window.innerWidth, g = window.innerHeight, p = 300, b = 100, m = 20;
      let y = h.left, v = h.top - b - 10;
      y + p > u - m && (y = u - p - m), y < m && (y = m), v < m && (v = h.bottom + 10, v + b > g - m && (v = (g - b) / 2)), v + b > g - m && (v = g - b - m), y = Math.max(m, Math.min(y, u - p - m)), v = Math.max(m, Math.min(v, g - b - m)), n = { x: `${y}px`, y: `${v}px` };
    }
  ...
  showTabContextMenu(e, t) {
    ...
    this.savedTabSets.length > 0 && d.push({
      ...
      v.textContent = p.text, b.appendChild(v), p.disabled || (b.addEventListener("mouseenter", () => {
  ...
  async restorePosition() {
    try {
      this.position = Y(
        ...
        this.verticalPosition,
        this.horizontalPosition
      ), this.position = Le(this.position, this.isVerticalMode, this.verticalWidth), this.log(`📍 位置已恢复: ${ye(this.position, this.isVerticalMode)}`);
    } catch {
      this.warn("无法恢复标签位置");
    }
  }
  ...
  async handleNewBlockInPanel(e, t) {
    ...
    if (d) {
      if (d.isPinned) {
        ...
        const m = o.filter((y) => y.isPinned).length;
        ...
        if (g.type === "attributes" && g.attributeName === "class") {
          ...
          if (p.classList.contains("orca-panel")) {
            if (c = !0, p.classList.contains("active")) {
              ...
              let y = null;
              m.forEach((v) => {
                const T = v.classList.contains("orca-hideable-hidden"), x = v.querySelector(".orca-block-editor[data-block-id]"), k = x == null ? void 0 : x.getAttribute("data-block-id");
                !T && x && k && (y = k);
  ...
  createRecentlyClosedTabsMenu(e, t) {
    ...
    a && a.remove();
    const r = document.documentElement.classList.contains("dark") || ((b = (p = window.orca) == null ? void 0 : p.state) == null ? void 0 : b.themeMode) === "dark", i = document.createElement("div");
    ...
    const g = (m) => {
      i.contains(m.target) || (i.remove(), document.removeEventListener("click", g), document.removeEventListener("contextmenu", g));
    };
    setTimeout(() => {
  ...
  showSaveTabSetDialog() {
    ...
    y.value = "", y.textContent = "请选择标签页集合...", m.appendChild(y), this.savedTabSets.forEach((E, R) => {
      const F = document.createElement("option");
      F.value = R.toString(), F.textContent = `${E.name} (${E.tabs.length}个标签)`, m.appendChild(F);
    }), p.appendChild(m), r.appendChild(h), r.appendChild(p), t.appendChild(r);
    const v = document.createElement("div");
    v.style.cssText = `
      padding: 20px;
      display: flex;
    ...
    x.onclick = async () => {
      if (c) {
        ...
        if (isNaN(E) || E < 0 || E >= this.savedTabSets.length) {
          orca.notify("warn", "请选择要更新的标签页集合");
          return;
        }
        t.remove(), await this.performUpdateTabSet(E);
      } else {
        ...
        t.remove(), await this.performSaveTabSet(E);
      }
    }, v.appendChild(T), v.appendChild(x), t.appendChild(v), document.body.appendChild(t), setTimeout(() => {
      g.focus(), g.select();
    }, 100), g.addEventListener("keydown", (E) => {
      E.key === "Enter" ? (E.preventDefault(), x.click()) : E.key === "Escape" && (E.preventDefault(), T.click());
    });
    const I = (E) => {
      t.contains(E.target) || (t.remove(), document.removeEventListener("click", I));
    };
    setTimeout(() => {
  ...
  showSaveWorkspaceDialog() {
    ...
    }, d.appendChild(h), d.appendChild(u), r.appendChild(i), r.appendChild(n), r.appendChild(o), r.appendChild(c), r.appendChild(l), r.appendChild(d), a.appendChild(r), document.body.appendChild(a), o.focus(), a.addEventListener("click", (m) => {
      m.target === a && a.remove();
    });
    const g = (m) => {
  ...
  showWorkspaceMenu(e) {
    ...
    t && t.remove();
    const a = document.documentElement.classList.contains("dark") || ((T = (v = window.orca) == null ? void 0 : v.state) == null ? void 0 : T.themeMode) === "dark", r = document.createElement("div");
    ...
    `, this.workspaces.length === 0) {
      ...
      this.workspaces.forEach((x) => {
        ...
        `;
        const I = x.icon || "ti ti-folder";
        k.innerHTML = `
          <i class="${I}" style="font-size: 14px; color: var(--orca-color-primary-5);"></i>
          <div style="flex: 1;">
    ...
    r.appendChild(d), r.appendChild(h), r.appendChild(g), r.appendChild(p), m && r.appendChild(m), document.body.appendChild(r);
    const y = (x) => {
      r.contains(x.target) || (r.remove(), document.removeEventListener("click", y));
    };
    setTimeout(() => {
  ...
  async editTabSetIcon(e, t, a, r, i) {
    ...
    `, l.forEach((p) => {
      ...
      `, p.value.startsWith("ti ti-")) {
        ...
        v.className = p.value, m.appendChild(v);
      } else
      ...
      y.style.cssText = `
        ...
        color: #666;
        text-align: center;
      `, y.textContent = p.name, b.appendChild(m), b.appendChild(y), b.addEventListener("click", async (v) => {
        v.stopPropagation(), e.icon = p.value, e.updatedAt = Date.now(), await this.saveSavedTabSets(), r(), n.remove(), i && i.focus(), orca.notify("success", "图标已更新");
  ...
  async manageSavedTabSets() {
    ...
    `, this.savedTabSets.forEach((c, l) => {
      ...
      }, y.appendChild(v), y.appendChild(T), y.appendChild(x), d.appendChild(h), d.appendChild(y), r.appendChild(d);
    }), t.appendChild(r);
    const i = document.createElement("div");
    i.style.cssText = `
      padding: 20px;
...
let P = null;
async function $a(s) {
  V = s, orca.state.locale, P = new Sa(V), document.readyState === "loading" ? document.addEventListener("DOMContentLoaded", () => {
