...
function gt(r, t) {
  var d, u, h, g;
  const e = E(r, t == null ? void 0 : t.in), a = e.getFullYear(), n = V(), i = (t == null ? void 0 : t.firstWeekContainsDate) ?? ((u = (d = t == null ? void 0 : t.locale) == null ? void 0 : d.options) == null ? void 0 : u.firstWeekContainsDate) ?? n.firstWeekContainsDate ?? ((g = (h = n.locale) == null ? void 0 : h.options) == null ? void 0 : g.firstWeekContainsDate) ?? 1, o = P((t == null ? void 0 : t.in) || r, 0);
...
function M(r, t, e) {
  ...
  let c = t.match(ke).map((p) => {
    const m = p[0];
    if (m === "p" || m === "P") {
      const b = pe[m];
      return b(p, n.formatLong);
    }
    return p;
  }).join("").match(we).map((p) => {
    if (p === "''")
...
`)[0].trim();
    ...
    try {
      ...
      if (this.addNewTabButton(), this.addWorkspaceButton(), this.isFixedToTop) {
        ...
        d.forEach((h) => {
          ...
          if (!g) return;
          const m = this.getCurrentPanelTabs().find((b) => b.blockId === g);
          if (m) {
            let b, x, v = "normal";
            if (b = "var(--orca-tab-bg)", x = "var(--orca-color-text-1)", m.color)
              try {
                h.style.setProperty("--tab-color", m.color), b = "var(--orca-tab-colored-bg)", x = "var(--orca-tab-colored-text)", v = "600";
              } catch {
              }
            h.style.cssText = `
            display: flex;
  ...
  showNewTabButtonContextMenu(t) {
    ...
    this.isFixedToTop && l.push(
      ...
      m.textContent = g.text, p.appendChild(m), p.addEventListener("mouseenter", () => {
   ...
   */
  rgbToOklch(t, e, a) {
    const n = t / 255, i = e / 255, o = a / 255, s = (Y) => Y <= 0.04045 ? Y / 12.92 : Math.pow((Y + 0.055) / 1.055, 2.4), c = s(n), l = s(i), d = s(o), u = c * 0.4124564 + l * 0.3575761 + d * 0.1804375, h = c * 0.2126729 + l * 0.7151522 + d * 0.072175, g = c * 0.0193339 + l * 0.119192 + d * 0.9503041, p = 0.2104542553 * u + 0.793617785 * h - 0.0040720468 * g, m = 1.9779984951 * u - 2.428592205 * h + 0.4505937099 * g, b = 0.0259040371 * u + 0.7827717662 * h - 0.808675766 * g, x = Math.cbrt(p), v = Math.cbrt(m), T = Math.cbrt(b), k = 0.2104542553 * x + 0.793617785 * v + 0.0040720468 * T, S = 1.9779984951 * x - 2.428592205 * v + 0.4505937099 * T, $ = 0.0259040371 * x + 0.7827717662 * v - 0.808675766 * T, C = Math.sqrt(S * S + $ * $), N = Math.atan2($, S) * 180 / Math.PI, O = N < 0 ? N + 360 : N;
    return { l: k, c: C, h: O };
  }
  /**
   * OKLCH转RGB颜色空间
   */
  oklchToRgb(t, e, a) {
    const n = a * Math.PI / 180, i = e * Math.cos(n), o = e * Math.sin(n), s = t, c = i, l = o, d = s * s * s, u = c * c * c, h = l * l * l, g = 1.0478112 * d + 0.0228866 * u - 0.050217 * h, p = 0.0295424 * d + 0.9904844 * u + 0.0170491 * h, m = -92345e-7 * d + 0.0150436 * u + 0.7521316 * h, b = 3.2404542 * g - 1.5371385 * p - 0.4985314 * m, x = -0.969266 * g + 1.8760108 * p + 0.041556 * m, v = 0.0556434 * g - 0.2040259 * p + 1.0572252 * m, T = (C) => C <= 31308e-7 ? 12.92 * C : 1.055 * Math.pow(C, 1 / 2.4) - 0.055, k = Math.max(0, Math.min(255, Math.round(T(b) * 255))), S = Math.max(0, Math.min(255, Math.round(T(x) * 255))), $ = Math.max(0, Math.min(255, Math.round(T(v) * 255)));
    return { r: k, g: S, b: $ };
  }
  /**
   * 应用颜色调整（支持亮色和暗色模式）
  ...
  showOrcaRenameInput(t) {
    ...
    let o = { x: "50%", y: "50%" };
    if (i) {
      const u = i.getBoundingClientRect(), h = window.innerWidth, g = window.innerHeight, p = 300, m = 100, b = 20;
      let x = u.left, v = u.top - m - 10;
      x + p > h - b && (x = h - p - b), x < b && (x = b), v < b && (v = u.bottom + 10, v + m > g - b && (v = (g - m) / 2)), v + m > g - b && (v = g - m - b), x = Math.max(b, Math.min(x, h - p - b)), v = Math.max(b, Math.min(v, g - m - b)), o = { x: `${x}px`, y: `${v}px` };
    }
  ...
  async handleNewBlockInPanel(t, e) {
    ...
    if (!t || !e) return;
    const a = this.getPanelIds().indexOf(e);
    a !== -1 && (this.currentPanelIndex = a, this.currentPanelId = e);
    let n = this.getCurrentPanelTabs();
    const i = n.find((h) => h.blockId === t);
    if (i) {
      this.closedTabs.has(t) && (this.closedTabs.delete(t), this.saveClosedTabs()), this.updateFocusState(t, i.title), this.immediateUpdateTabsUI();
      return;
    }
  ...
  async checkCurrentPanelBlocks() {
    ...
    if (!t) {
      ...
      this.log("📊 当前所有面板状态:"), g.forEach((p, m) => {
        const b = p.getAttribute("data-panel-id"), x = p.classList.contains("active");
        this.log(`  面板${m + 1}: ID=${b}, active=${x}`);
      });
      return;
    }
    const e = t.getAttribute("data-panel-id");
    ...
    if (d === -1)
      return;
    const u = await this.getTabInfo(i, e, d);
    u && (o[d] = u, this.setCurrentPanelTabs(o), await this.immediateUpdateTabsUI());
  }
  /**
   * 监听DOM变化的核心方法
   * 
  ...
  observeChanges() {
    new MutationObserver(async (n) => {
      ...
      n.forEach((h) => {
        if (h.type === "childList") {
          const g = h.target;
          if ((g.classList.contains("orca-panels-row") || g.closest(".orca-panels-row")) && (o = !0), h.addedNodes.length > 0 && g.closest(".orca-panel")) {
            for (const m of h.addedNodes)
              if (m.nodeType === Node.ELEMENT_NODE) {
                const b = m;
                if (this.handleNewHideableElement(b)) {
                  i = !0;
                  break;
                }
                if (b.classList.contains("orca-block-editor") || b.querySelector(".orca-block-editor")) {
                  i = !0;
                  break;
                }
                if (this.handleChildHideableElements(b)) {
                  i = !0;
                  break;
                }
              }
        ...
        if (h.type === "attributes" && h.attributeName === "class") {
          ...
          if (g.classList.contains("orca-panel")) {
            if (s = !0, g.classList.contains("active")) {
              const p = g.getAttribute("data-panel-id"), m = g.querySelectorAll(".orca-hideable");
              let b = null;
              m.forEach((x) => {
                ...
                !v && T && k && (b = k);
              }), b && p && this.handleNewBlockInPanel(b, p).catch((x) => {
                this.error(`处理面板激活时的新块失败: ${b}`, x);
              }), setTimeout(async () => {
    ...
    `, t.forEach((h, g) => {
      ...
      `, h.icon) {
        ...
        `, h.icon.startsWith("ti ti-")) {
          ...
          x.className = h.icon, b.appendChild(x);
        } else
  ...
  showSaveTabSetDialog() {
    ...
    let c = !1;
    const l = () => {
      c = !1, o.className = "orca-button orca-button-secondary", o.style.cssText = "flex: 1;", s.className = "orca-button", s.style.cssText = "flex: 1;", u.style.display = "block", p.style.display = "none", S();
    }, d = () => {
      c = !0, s.className = "orca-button orca-button-secondary", s.style.cssText = "flex: 1;", o.className = "orca-button", o.style.cssText = "flex: 1;", u.style.display = "none", p.style.display = "block", S();
    };
    ...
    x.value = "", x.textContent = "请选择标签页集合...", b.appendChild(x), this.savedTabSets.forEach((C, N) => {
      const O = document.createElement("option");
      O.value = N.toString(), O.textContent = `${C.name} (${C.tabs.length}个标签)`, b.appendChild(O);
    }), p.appendChild(b), n.appendChild(u), n.appendChild(p), e.appendChild(n);
  ...
  renderSortableTabs(t, e, a) {
    ...
    e.forEach((c, l) => {
      ...
      `, m.textContent = (l + 1).toString(), p.appendChild(m), d.appendChild(p), d.addEventListener("dragstart", (b) => {
        ...
        const x = parseInt(b.dataTransfer.getData("text/plain")), v = l;
  ...
  showSaveWorkspaceDialog() {
    ...
    }, d.appendChild(u), d.appendChild(h), n.appendChild(i), n.appendChild(o), n.appendChild(s), n.appendChild(c), n.appendChild(l), n.appendChild(d), a.appendChild(n), document.body.appendChild(a), s.focus(), a.addEventListener("click", (b) => {
      b.target === a && a.remove();
    });
    const g = (b) => {
  ...
  renameTabSet(t, e, a) {
    ...
    h.className = "orca-button orca-button-primary", h.textContent = "保存", h.style.cssText = "", h.addEventListener("mouseenter", () => {
      ...
      t.name = p, t.updatedAt = Date.now(), await this.saveSavedTabSets(), i.remove(), a.remove(), this.manageSavedTabSets(), orca.notify("success", "重命名成功");
  ...
  async editTabSetIcon(t, e, a, n, i) {
    ...
    `, l.forEach((p) => {
      ...
      x.style.cssText = `
        ...
        color: #666;
        text-align: center;
      `, x.textContent = p.name, m.appendChild(b), m.appendChild(x), m.addEventListener("click", async (v) => {
        v.stopPropagation(), t.icon = p.value, t.updatedAt = Date.now(), await this.saveSavedTabSets(), n(), o.remove(), i && i.focus(), orca.notify("success", "图标已更新");
  ...
  async manageSavedTabSets() {
    ...
    `, this.savedTabSets.forEach((c, l) => {
      ...
      }, x.appendChild(v), x.appendChild(T), x.appendChild(k), d.appendChild(u), d.appendChild(x), n.appendChild(d);
    }), e.appendChild(n);
    const i = document.createElement("div");
    i.style.cssText = `
      padding: 20px;
