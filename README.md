...
### v2.2.3
- 🐛 **改进标签页拖拽排序功能** - 解决拖动移动标签页顺序有时候会拖不过去的问题
- 🔧 **优化防抖机制** - 使用16ms延迟（一帧时间）确保拖拽事件稳定
- 🔄 **简化交换逻辑** - 直接交换位置避免复杂计算，提高拖拽成功率
- 🛡️ **增强事件处理** - 添加stopPropagation防止事件冒泡，避免与其他元素冲突
- 📏 **改进边界检测** - 增加5px容错范围避免快速移动时误判
- ⚡ **优化状态管理** - 清除定时器避免状态混乱，提高拖拽体验稳定性

### v2.2.2
- 🐛 **重构垂直布局侧边栏对齐功能** - 修复垂直布局下侧边栏对齐功能的多个bug
- 🔧 **修复动态位置读取** - 优先从DOM元素动态读取当前位置，确保对齐准确性
- 🛡️ **修复Y坐标重置问题** - 只更新X坐标，保持Y坐标不变，保护用户自定义位置
- 🔄 **统一对齐逻辑** - 水平和垂直模式使用相同的核心对齐方法
- 📍 **修复位置管理** - 修复updateUIPositions和constrainPosition方法
- ⚡ **代码架构优化** - 消除约200行重复代码，提升维护性
