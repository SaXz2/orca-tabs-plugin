...
export class OrcaStorageService {
    ...
    try {
      ...
      try {
        if (typeof localStorage !== 'undefined') {
          localStorage.setItem(`${pluginName}:${key}`, serializedData);
          this.log(`Fallback save to localStorage for ${key}`);
          return true;
        }
      } catch (storageError) {
        this.error(`localStorage ???????????? ${key}:`, storageError);
      }
  ...
  async getConfig<T>(key: string, pluginName: string = 'orca-tabs-plugin', defaultValue?: T): Promise<T | null> {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        const stored = localStorage.getItem(`${pluginName}:${key}`);
  ...
  async removeConfig(key: string, pluginName: string = 'orca-tabs-plugin'): Promise<boolean> {
    try {
      ...
      try {
        if (typeof localStorage !== 'undefined') {
          localStorage.removeItem(`${pluginName}:${key}`);
          this.log(`Fallback remove from localStorage for ${key}`);
          return true;
        }
      } catch (storageError) {
        this.error(`localStorage ???????????? ${key}:`, storageError);
