...
class OrcaTabsPlugin {
  ...
  showSaveTabSetDialog() {
    ...
    updateExistingBtn.style.cssText = `
      flex: 1;
      padding: 8px 12px;
      border: 1px solid #ddd;
      background: white;
  ...
  renderSortableTabs(container: HTMLElement, tabs: TabInfo[], tabSet?: SavedTabSet) {
    ...
    tabs.forEach((tab, index) => {
      ...
      // 构建标签信息HTML
      let tabInfoHTML = `
        <div style="font-size: 14px; color: #333; font-weight: 500; line-height: 1.2; margin-bottom: 2px;">${tab.title}</div>
  ...
  private async clearCurrentWorkspace() {
    try {
      ...
      await this.storageService.saveConfig(PLUGIN_STORAGE_KEYS.CURRENT_WORKSPACE, null);
      this.log(`📁 已清除当前工作区状态`);
    } catch (error) {
  ...
  private async switchToWorkspace(workspaceId: string) {
    try {
      ...
      // 保存当前工作区ID到存储
      await this.storageService.saveConfig(PLUGIN_STORAGE_KEYS.CURRENT_WORKSPACE, workspaceId);
